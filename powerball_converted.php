<?php
/**
 * LottoExpert.net — Powerball Results Intelligence Page
 * Arkansas • Joomla 5.x + PHP 8.1+
 *
 * GAME CONFIG:
 * - game_id  = 101  (Powerball main)
 * - game_id  = 101D (Powerball Double Play)
 * - main balls = 5, range 01..69
 * - Powerball range = 01..26
 */
...
$stateAbrev = 'ar';
$gName      = 'Powerball';
$gName2     = 'Powerball Double Play';
$stLink     = $input->getString('stn', 'ar');
...
$doc->setTitle('Powerball Results, Stats &amp; Analysis | LottoExpert.net');
$doc->setDescription('See today\'s Powerball results plus number frequencies, hot and cold stats, and overdue numbers updated after every draw on LottoExpert.net.');
...
/**
 * Powerball Double Play — latest result
 */
$dpRows = leFetchRecentDraws($db, (string) $dbCol, $gameIdDp, 1);
$dpRow  = !empty($dpRows) ? $dpRows[0] : null;
$dp1    = $dpRow ? trim((string) ($dpRow['first']  ?? '')) : '';
$dp2    = $dpRow ? trim((string) ($dpRow['second'] ?? '')) : '';
$dp3    = $dpRow ? trim((string) ($dpRow['third']  ?? '')) : '';
$dp4    = $dpRow ? trim((string) ($dpRow['fourth'] ?? '')) : '';
$dp5    = $dpRow ? trim((string) ($dpRow['fifth']  ?? '')) : '';
$dpb    = $dpRow ? trim((string) ($dpRow['sixth']  ?? '')) : '';
$dpDate = $dpRow ? (string) ($dpRow['draw_date'] ?? '') : '';

/**
 * Build frequency windows
 * Main white balls: 01-69
 * Powerball:        01-26
 */
...
[$bonusCounts, $bonusLastSeenIndex] = leInitRange(1, 26);
...
$bonusChartLabels = leBuildNaturalLabels(1, 26);
$bonusChartValues = [];
...
/**
 * Draw history for current draw balls
 */
$drawHistoryRows = [];
 ...
 * Copy / CTA
 */
$heroInsight  = 'Latest verified Powerball draw and recent number behavior at a glance. Review the most active white balls, quiet stretches, and full historical frequency before moving into deeper SKAI analysis.';
...
.skai-dp-ball--main{
  background:linear-gradient(180deg,#e0e8ff 0%,#b8c8f0 100%);
  color:var(--deep-navy);
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            ...
            <a class="skai-btn skai-btn--secondary" href="/powerball/quick-pick-today/?game_id=101&amp;stn=<?php echo rawurlencode($stLink); ?>">
              Quick Picks Today
            </a>
        ...
        <aside class="skai-result-panel" aria-label="Latest draw details">
          ...
          <div class="skai-meta-stack">
            ...
            <?php if ($nextJackpot !== '' && (float) $nextJackpot > 0 && strtolower($nextJackpot) !== 'n/a') : ?>
              <div class="skai-meta-box">
            ...
            <?php if ($dpRow !== null && $dp1 !== '') : ?>
              <div class="skai-meta-box">
                <span class="label">Double Play &mdash; <?php echo htmlspecialchars(leFmtDate($dpDate), ENT_QUOTES, 'UTF-8'); ?></span>
                <div class="skai-dp-ball-row" aria-label="Powerball Double Play numbers">
                  <span class="skai-dp-ball skai-dp-ball--main"><?php echo htmlspecialchars(lePad2($dp1), ENT_QUOTES, 'UTF-8'); ?></span>
                  <span class="skai-dp-ball skai-dp-ball--main"><?php echo htmlspecialchars(lePad2($dp2), ENT_QUOTES, 'UTF-8'); ?></span>
                  <span class="skai-dp-ball skai-dp-ball--main"><?php echo htmlspecialchars(lePad2($dp3), ENT_QUOTES, 'UTF-8'); ?></span>
                  <span class="skai-dp-ball skai-dp-ball--main"><?php echo htmlspecialchars(lePad2($dp4), ENT_QUOTES, 'UTF-8'); ?></span>
                  <span class="skai-dp-ball skai-dp-ball--main"><?php echo htmlspecialchars(lePad2($dp5), ENT_QUOTES, 'UTF-8'); ?></span>
                  <span class="skai-ball-gap" aria-hidden="true"></span>
                  <span class="skai-dp-ball skai-dp-ball--bonus"><?php echo htmlspecialchars(lePad2($dpb), ENT_QUOTES, 'UTF-8'); ?></span>
                </div>
              </div>
            <?php endif; ?>
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        ...
        <div class="skai-stat-note">White-ball numbers from the latest draw that also appeared in the recent trailing draws.</div>
      </div>
    </article>
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        ...
        <div class="skai-stat-note">White-ball and Powerball draw windows currently loaded for this page view.</div>
      </div>
    </article>
  </section>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This section makes the current Powerball draw easier to interpret. It shows how recently each latest number was previously seen and how the perspective shifts when you compare a short recent window with a broader historical one.
        </p>
      </div>
    </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel shows the complete white-ball distribution across all values 01&ndash;69. The second panel shows the Powerball distribution for values 01&ndash;26 and the recency spread for each white-ball number.
        </p>
      </div>
    </div>
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              ...
              <span class="skai-card-sub">All values 01&ndash;26 across the last <?php echo (int) $nodCurrentBonus; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame">
                  <canvas id="powballChart" aria-label="Powerball distribution chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="skai-card">
            ...
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame">
                  <canvas id="recencyChart" aria-label="White-ball recency chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
            ...
            <table id="skai-main-table" class="skai-table" aria-label="White-ball frequency table">
              ...
              <tbody>
                <?php for ($i = 1; $i <= 69; $i++) : ?>
                  ...
                  list($lastDrawSort, $lastDrawLabel) = leDrawingsAgoLabel($mainLastSeenIndex[$number] ?? null, (int) $nodCurrentMain);
        ...
        <div class="skai-card">
          ...
          <div class="skai-table-wrap">
            <table id="skai-bonus-table" class="skai-table" aria-label="Powerball frequency table">
              <thead>
                <tr>
                  <th>Number</th>
                  <th>Drawn Times</th>
              ...
              <tbody>
                <?php for ($i = 1; $i <= 26; $i++) : ?>
                  ...
                  list($lastDrawSort, $lastDrawLabel) = leDrawingsAgoLabel($bonusLastSeenIndex[$number] ?? null, (int) $nodCurrentBonus);
                  ?>
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        type="checkbox"
                        value="<?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                        aria-label="Track Powerball number <?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                      >
                    </td>
                  </tr>
                <?php endfor; ?>
  ...
  <section id="tools" class="skai-section" aria-labelledby="tools-title">
    ...
    <div class="skai-section-body">
      ...
      <div class="skai-utility-grid">
        ...
        <a class="skai-utility-link" href="/lottery-archives?gId=101&amp;sTn=ar">Powerball Archives</a>
        <a class="skai-utility-link" href="/powerball/quick-pick-today/?game_id=101&amp;stn=<?php echo rawurlencode($stLink); ?>">Quick Picks Today</a>
        <a class="skai-utility-link" href="/powerball/lucky-numbers-today/">Lucky Numbers Today</a>
        <a class="skai-utility-link" href="/lowest-drawn-number-analysis?gId=<?php echo rawurlencode($gId); ?>&amp;stateName=<?php echo rawurlencode($stateName); ?>&amp;gName=<?php echo rawurlencode($gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower($stateAbrev)); ?>">Lowest Number Threshold</a>
      </div>
    </div>
  </section>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This page is designed to help users understand recent and historical Powerball behavior more clearly, not to imply certainty.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. Lottery outcomes are independent events. The purpose of this page is to make the recent behavior of Powerball easier to understand, compare, and carry into deeper SKAI analysis.
      </div>
    </div>
  </section>
...
(function () {
  ...
  var chartData = {
    ...
    topActiveValues:  <?php echo json_encode(array_values($topActiveValues),  JSON_UNESCAPED_UNICODE); ?>,
    quietLabels:      <?php echo json_encode(array_values($quietestLabels),   JSON_UNESCAPED_UNICODE); ?>,
    quietValues:      <?php echo json_encode(array_values($quietestValues),   JSON_UNESCAPED_UNICODE); ?>,
    mainLabels:       <?php echo json_encode(array_values($mainChartLabels),  JSON_UNESCAPED_UNICODE); ?>,
    mainValues:       <?php echo json_encode(array_values($mainChartValues),  JSON_UNESCAPED_UNICODE); ?>,
    mainRecencyValues:<?php echo json_encode(array_values($mainRecencyValues),JSON_UNESCAPED_UNICODE); ?>,
    bonusLabels:      <?php echo json_encode(array_values($bonusChartLabels), JSON_UNESCAPED_UNICODE); ?>,
    bonusValues:      <?php echo json_encode(array_values($bonusChartValues), JSON_UNESCAPED_UNICODE); ?>
  };
  ...
  function renderCharts() {
    if (!window.Chart || chartsRendered) { return; }
    chartsRendered = true;

    var ctxTopActive = safeContext('topActiveChart');
    var ctxQuiet     = safeContext('quietChart');
    var ctxFullMain  = safeContext('fullMainChart');
    var ctxPowball   = safeContext('powballChart');
    var ctxRecency   = safeContext('recencyChart');

    if (ctxTopActive && chartData.topActiveLabels.length) {
    ...
    if (ctxQuiet && chartData.quietLabels.length) {
      new Chart(ctxQuiet, {
        type: 'bar',
        data: {
    ...
    if (ctxFullMain && chartData.mainLabels.length) {
    ...
    if (ctxPowball && chartData.bonusLabels.length) {
      new Chart(ctxPowball, {
        type: 'bar',
        data: {
    ...
    if (ctxRecency && chartData.mainLabels.length) {
